const ind = {
  // ── Common ──
  common: {
    appName: "Wiz Scheduler",
    loading: "Memuat...",
    save: "Simpan",
    saving: "Menyimpan...",
    cancel: "Batal",
    edit: "Edit",
    delete: "Hapus",
    remove: "Hapus",
    add: "Tambah",
    create: "Buat",
    update: "Perbarui",
    import: "Impor",
    export: "Ekspor",
    done: "Selesai",
    close: "Tutup",
    actions: "Tindakan",
    name: "Nama",
    email: "Email",
    password: "Kata Sandi",
    date: "Tanggal",
    start: "Mulai",
    end: "Selesai",
    startTime: "Waktu Mulai",
    endTime: "Waktu Selesai",
    role: "Peran",
    roles: "Peran",
    status: "Status",
    employee: "Karyawan",
    employees: "Karyawan",
    location: "Lokasi",
    locations: "Lokasi",
    company: "Perusahaan",
    companies: "Perusahaan",
    description: "Deskripsi",
    none: "Tidak ada",
    auto: "otomatis",
    select: "-- pilih --",
    yes: "Ya",
    no: "Tidak",
    of: "dari",
    selected: "dipilih",
    day: "hari",
    days: "hari",
    shift: "shift",
    shifts: "shift",
    week: "Minggu",
    logout: "Keluar",
    dismiss: "tutup",
    errors: "Kesalahan",
    warnings: "Peringatan",
  },

  // ── Language selector ──
  langSelector: {
    label: "Bahasa",
  },

  // ── Sidebar nav ──
  nav: {
    dashboard: "Dasbor",
    company: "Perusahaan",
    team: "Team",
    specialHours: "Special Hours",
    regions: "Wilayah",
    locations: "Lokasi",
    roles: "Peran",
    roleEquivalents: "Kesetaraan Peran",
    employees: "Karyawan",
    hourRestrictions: "Hour Restrictions",
    dayBlackouts: "Day Rules",
    employeeOnboarding: "Orientasi Karyawan",
    employeeAvailability: "Employee Availability",
    employeeAssociation: "Ketersediaan & Asosiasi Karyawan",
    shiftTemplates: "Template Shift",
    schedule: "Jadwal",
    exportSchedules: "Ekspor Jadwal yang Disetujui",
    myAvailability: "Ketersediaan Saya",
    dataPrivacy: "Data & Privacy",
    checkInQr: "Kode Check-in",
    checkInReport: "Laporan Check-in",
    payroll: "Penggajian",
    checkIn: "Check-in",
    groupOrganization: "Organisasi",
    groupRoles: "Peran & Keahlian",
    groupPeople: "Orang",
    groupSchedulingRules: "Aturan Penjadwalan",
    groupScheduling: "Penjadwalan",
    groupCheckIn: "Check-in",
    approvedSchedules: "Jadwal Disetujui",
    dayPreferences: "Preferensi Hari",
    hourRangePreferences: "Preferensi Rentang Jam",
    frequencyCaps: "Batas Frekuensi",
  },

  // ── Team ──
  approvedSchedules: {
    title: "Jadwal Disetujui",
    subtitle: "Lihat minggu sebelumnya persis seperti saat dibuat.",
    weekOf: "Minggu",
    dotLegend: "memiliki jadwal disetujui",
    emptyWeek: "Tidak ada jadwal disetujui untuk minggu ini.",
    loadError: "Tidak dapat memuat jadwal minggu ini.",
    previousMonth: "Bulan sebelumnya",
    nextMonth: "Bulan berikutnya",
    editTitle: "Edit Shift",
    readOnlyNotice: "Hanya-baca — jendela edit 30 hari telah berakhir.",
    savedWithWarnings: "Disimpan, dengan peringatan",
    deletedWithWarnings: "Dihapus, dengan peringatan",
    warningAlreadyBookedTitle: "Jadwal ganda",
    warningAlreadyBookedBody: "Karyawan ini sudah dijadwalkan untuk shift yang tumpang tindih.",
    warningNoAvailabilityTitle: "Tidak ada data ketersediaan",
    warningNoAvailabilityBody: "Karyawan ini tidak memiliki catatan ketersediaan yang mencakup shift ini.",
    warningAlreadyExportedTitle: "Sudah diekspor",
    warningAlreadyExportedBody: "Jadwal ini sudah diekspor; sistem eksternal mungkin kini tidak sinkron.",
    shiftLockedTitle: "Shift terkunci",
    shiftLockedBody: "Seorang karyawan sudah check-in untuk shift ini, sehingga tidak dapat diedit lagi.",
    scheduleLockedTitle: "Jadwal terkunci",
    scheduleLockedBody: "{name} sedang mengedit jadwal ini. Coba lagi sebentar.",
    invalidEdit: "Perubahan ini tidak dapat diterapkan. Tidak ada yang berubah.",
  },

  team: {
    title: "Team",
    inviteButton: "Invite manager",
    emailLabel: "Email",
    emailPlaceholder: "manager@example.com",
    sendInvite: "Send invite",
    cancel: "Cancel",
    pendingStatus: "Pending",
    acceptedStatus: "Accepted",
    expiredStatus: "Expired",
    columnEmail: "Email",
    columnStatus: "Status",
    columnInvitedAt: "Invited",
    columnAcceptedAt: "Accepted",
    columnCompany: "Company",
    noInvites: "No manager invites yet.",
  },

  // ── Login ──
  login: {
    title: "Masuk ke Wiz Scheduler",
    signingIn: "Sedang masuk...",
    signIn: "Masuk",
    noAccount: "Belum punya akun?",
    register: "Daftar",
    multiOrgPrompt: "Akun Anda terhubung dengan beberapa organisasi. Silakan pilih organisasi yang ingin Anda masuki:",
    backToLogin: "Kembali ke halaman masuk",
    demoCredentials: "Kredensial Demo",
    manager: "Manajer",
    employeeLabel: "Karyawan",
    orContinueWith: "or continue with",
    googleLinkPrompt: "An account with {email} already exists. Enter your password to link your Google account.",
    enterPasswordToLink: "Enter your password to link",
    linking: "Linking...",
    linkAndSignIn: "Link Google & Sign In",
    forgotPassword: "Forgot password?",
  },
  // ── Forgot Password ──
  forgotPassword: {
    title: "Forgot your password?",
    description: "Enter the email you sign in with and we'll send you a reset link. The link expires in 30 minutes.",
    emailPlaceholder: "you@example.com",
    submit: "Send reset link",
    submitting: "Sending…",
    failed: "Could not send reset link.",
    confirmTitle: "Check your inbox",
    confirmBody: "If an account exists for that email, a reset link is on its way. Didn't get it? Check spam or try again in a few minutes.",
    backToLogin: "Back to sign in",
  },

  // ── Reset Password ──
  resetPassword: {
    title: "Set a new password",
    description: "Choose a new password for your WizScheduler account. After saving, you'll be signed in automatically.",
    newPassword: "New password",
    confirmPassword: "Confirm new password",
    atLeast6: "At least 6 characters",
    reenterPassword: "Re-enter password",
    submit: "Save new password",
    submitting: "Saving…",
    passwordMin: "Password must be at least 6 characters.",
    passwordMismatch: "Passwords do not match.",
    expired: "This reset link has expired. Please request a new one.",
    invalidLink: "Invalid reset link",
    invalidLinkDesc: "This reset link is missing, malformed, or has already been used.",
    failed: "Could not save the new password.",
    backToLogin: "Back to sign in",
  },


  // ── Verify Email ──
  // English in every locale, matching how forgotPassword/resetPassword
  // above already ship. Translating the auth flows is a separate pass.
  verifyEmail: {
    bannerTitle: "Confirm your email",
    bannerBody:
      "Confirm your email address to generate schedules. We sent a link to {email} when you signed up.",
    resend: "Resend link",
    resending: "Sending\u2026",
    resent:
      "If that address still needs confirming, a new link is on its way. Check spam if it doesn't arrive.",
    verifying: "Confirming your email\u2026",
    successTitle: "Email confirmed",
    successBody:
      "Thanks \u2014 your address is confirmed. You can generate schedules now.",
    invalidTitle: "Link no longer valid",
    invalidBody:
      "This confirmation link has expired or has already been used. Sign in and request a new one.",
    goToDashboard: "Go to dashboard",
    backToLogin: "Back to sign in",
  },

  // ── Register ──
  register: {
    title: "Buat Akun",
    fullName: "Nama Lengkap",
    companyName: "Nama Perusahaan",
    creatingAccount: "Membuat akun...",
    registerBtn: "Daftar",
    haveAccount: "Sudah punya akun?",
    signIn: "Masuk",
    googleLinkNote: "After registering, you can link your Google account from the login page.",
    createFreeAccount: "Buat akun gratis",
  },

  // ── Accept Invite ──
  acceptInvite: {
    noToken: "Token undangan tidak tersedia.",
    invalidOrExpired: "Tautan undangan tidak valid atau sudah kedaluwarsa.",
    passwordMin: "Kata sandi harus minimal 6 karakter.",
    passwordMismatch: "Kata sandi tidak cocok.",
    failedCreate: "Gagal membuat akun.",
    invalidInvite: "Undangan Tidak Valid",
    invalidInviteDesc: "Tautan undangan ini tidak valid atau sudah kedaluwarsa.",
    goToLogin: "Ke Halaman Masuk",
    setupTitle: "Siapkan Akun Anda",
    welcomeTo: "Selamat datang di",
    choosePassword: "Pilih kata sandi untuk mengaktifkan akun Anda.",
    confirmPassword: "Konfirmasi Kata Sandi",
    atLeast6: "Minimal 6 karakter",
    reenterPassword: "Masukkan ulang kata sandi",
    creatingAccount: "Membuat Akun...",
    createAndLogin: "Buat Akun & Masuk",
    haveAccount: "Sudah punya akun?",
    logIn: "Masuk",
  },

  acceptManagerInvite: {
    title: "Accept Manager Invite",
    loadingInvite: "Loading invite...",
    noToken: "No invite token provided.",
    invalidOrExpired: "This invite link is invalid or has expired.",
    invalidInvite: "Invalid Invite",
    goToLogin: "Go to Login",
    welcomeTo: "You have been invited to manage",
    choosePassword: "Set up your account to start managing.",
    emailLabel: "Email",
    fullNameLabel: "Full name",
    fullNameRequired: "Full name is required.",
    companyLabel: "Company to manage",
    companyRequired: "Please select a company.",
    passwordLabel: "Password",
    confirmPasswordLabel: "Confirm password",
    passwordMin: "Password must be at least 6 characters.",
    passwordMismatch: "Passwords do not match.",
    atLeast6: "At least 6 characters",
    reenterPassword: "Re-enter password",
    submitButton: "Accept invite",
    submitting: "Setting up account...",
    failedCreate: "Failed to create account.",
    haveAccount: "Already have an account?",
    logIn: "Log in",
  },

  // ── Dashboard ──
  dashboard: {
    welcome: "Selamat datang,",
    manager: "Manajer",
    importFrom7shifts: "Impor dari 7shifts",
    importFromDeputy: "Import from Deputy",
    importDisabledFreePlan: "Tersedia di paket berbayar — upgrade untuk mengimpor secara otomatis.",
    upgradeConfirmFailed: "Tidak dapat mengonfirmasi peningkatan Anda. Jika Anda telah dikenakan biaya, hubungi dukungan dan kami akan menyelesaikannya.",
    subtitle: "Kelola penjadwalan Anda dari dasbor di bawah.",
    companyDesc: "Lihat dan edit pengaturan perusahaan",
    regionsDesc: "Kelola wilayah geografis",
    locationsDesc: "Kelola lokasi toko",
    rolesDesc: "Tentukan peran karyawan",
    employeesDesc: "Kelola tenaga kerja Anda",
    shiftTemplatesDesc: "Konfigurasi pola shift mingguan",
    scheduleDesc: "Buat dan setujui jadwal",
  },

  // ── Plan banner (free tier) ──
  planBanner: {
    reasonPlanLimitExceeded: "Akun Anda melebihi batas paket gratis. Upgrade untuk melanjutkan pembuatan jadwal.",
    reasonSubscriptionCanceled: "Langganan Anda telah dibatalkan. Aktifkan kembali untuk melanjutkan penjadwalan.",
    reasonAiRequiresPaid: "Penjadwalan AI memerlukan paket berbayar. Anda tetap dapat membuat jadwal tanpa AI.",
    reasonScheduleLimitReached: "Anda telah menggunakan semua pembuatan jadwal gratis untuk bulan ini. Batas akan direset pada awal bulan depan — upgrade untuk menghapusnya.",
    usage: "{employeeCount} dari {employeeLimit} karyawan · {locationCount} dari {locationLimit} lokasi",
    usageGenerations: " · {generationCount} dari {generationLimit} lokasi dijadwalkan bulan ini",
    upgradeButton: "Upgrade",
    upgrading: "Mengalihkan…",
    upgradeFailed: "Tidak dapat memulai checkout upgrade.",
  },

  // ── Link Google account (dashboard card) ──
  linkGoogle: {
    title: "Link your Google account",
    description: "Add Google sign-in so you can log in faster next time. Your Google account email must match this account.",
    linking: "Linking…",
    failed: "Could not link Google account.",
  },

  // ── Special Hours ──
  specialHours: {
    title: "Special Hours",
    description: "Days with non-standard operating hours — the scheduler will use the linked template instead of the regular weekly one.",
    addButton: "Add special hours",
    columnDate: "Date",
    columnLocation: "Location",
    columnHours: "Hours",
    columnLabel: "Label",
    columnTemplate: "Template",
    columnActions: "Actions",
    labelPlaceholder: "e.g. Christmas Eve, Thanksgiving",
    openLabel: "Open",
    closeLabel: "Close",
    draftTemplateLabel: "Starting draft template",
    draftTemplateHelp: "We'll clone this template for the special day. You can edit the clone afterwards.",
    duplicateTitle: "Duplicate to other locations",
    duplicateHelp: "Pick the locations to copy this special hours entry to. Each will get its own cloned template.",
    deleteConfirm: "Delete this special hours entry? The cloned template will also be removed.",
    noEntries: "No special hours configured yet.",
    noRecurringTemplate: "{location} has no recurring template — create one before adding special hours.",
    closeAfterOpen: "Close time must be after open time.",
    scheduleBadge: "Special hours",
    schedulePreviewTitle: "Special hours in this week",
    schedulePreviewHelp: "The scheduler will apply the linked override template on each of these days.",
    filterLocationLabel: "Location:",
    filterAllLocations: "All locations",
    editTitle: "Edit special hours",
    createTitle: "New special hours",
    saveButton: "Save",
    savingButton: "Saving…",
    cancelButton: "Cancel",
    editAction: "Edit",
    duplicateAction: "Duplicate",
    deleteAction: "Delete",
    duplicateConfirm: "Duplicated to {count} location(s).",
    dateLabel: "Date",
    locationLabel: "Location",
  },

  // ── Company ──
  companyPage: {
    title: "Pengaturan Perusahaan",
    companyName: "Nama Perusahaan",
    slug: "Slug",
    updateSuccess: "Nama perusahaan berhasil diperbarui.",
  },

  // ── Regions ──
  regions: {
    title: "Wilayah",
  },

  // ── Locations ──
  locationsPage: {
    title: "Lokasi",
    importData: "Impor Data",
    importTitle: "Impor Lokasi",
    address: "Alamat",
    timezone: "Zona Waktu",
    region: "Wilayah",
    limitReached: "Paket gratis mengizinkan {limit} lokasi — upgrade untuk menambah lebih banyak.",
    csvLimitError: "File ini berisi {rows} lokasi, tetapi paket gratis Anda hanya memiliki ruang untuk {remaining}. Seluruh file akan ditolak — upgrade, atau unggah file yang lebih kecil.",
  },

  // ── Roles ──
  rolesPage: {
    title: "Peran",
    importData: "Impor Data",
    importTitle: "Impor Peran",
    externalId: "ID Eksternal",
  },

  // ── Role Equivalents ──
  roleEquivalents: {
    title: "Kesetaraan Peran",
    description: "Kelompokkan peran yang memiliki tujuan penjadwalan yang sama. Karyawan dengan peran apa pun dalam grup dapat mengisi shift untuk peran lain dalam grup tersebut.",
    newGroup: "+ Grup Baru",
    editGroup: "Edit Grup",
    createGroup: "Buat Grup",
    condensedRoleName: "Nama Peran Ringkas",
    namePlaceholder: "mis. Pelayan, Resepsionis, Juru Masak",
    rolesInGroup: "Peran dalam grup ini",
    selectTwoOrMore: "(pilih 2 atau lebih)",
    allAssigned: "Semua peran sudah ditetapkan ke grup.",
    suggestedGroups: "Grup yang Disarankan",
    nameRequired: "Nama wajib diisi",
    selectAtLeast2: "Pilih minimal 2 peran untuk dikelompokkan",
    emptyState: "Belum ada kesetaraan peran yang dikonfigurasi. Buat grup agar karyawan dengan peran serupa dapat mengisi shift satu sama lain.",
  },

  // ── Employees ──
  employeesPage: {
    title: "Karyawan",
    importData: "Impor Data",
    importTitle: "Impor Karyawan",
    fullName: "Nama Lengkap",
    noRoles: "Tidak ada peran",
    skill: "Keahlian",
    addRow: "+ Tambah Baris",
    filterLocationLabel: "Location:",
    filterAllLocations: "All locations",
    limitReached: "Paket gratis mengizinkan {limit} karyawan — upgrade untuk menambah lebih banyak.",
    csvLimitError: "File ini berisi {rows} karyawan, tetapi paket gratis Anda hanya memiliki ruang untuk {remaining}. Seluruh file akan ditolak — upgrade, atau unggah file yang lebih kecil.",
  },

  // ── Employee Onboarding ──
  onboarding: {
    title: "Orientasi Karyawan",
    subtitle: "Kirim tautan undangan ke karyawan yang diimpor agar mereka dapat membuat akun dan mengelola ketersediaan mereka sendiri.",
    howItWorks: "Cara kerja orientasi",
    step1prefix: "Impor atau buat karyawan secara manual di halaman",
    step1link: "Karyawan",
    step1suffix: "(pastikan setiap karyawan memiliki email).",
    step2prefix: "Klik",
    step2bold: "Kirim Undangan",
    step2suffix: "di bawah untuk membuat tautan masuk unik. Tautan otomatis disalin ke clipboard Anda.",
    step3: "Bagikan tautan ke karyawan. Mereka akan membuat kata sandi, lalu dapat masuk untuk mengelola ketersediaan mereka.",
    expireNote: "Tautan undangan kedaluwarsa setelah 7 hari. Anda dapat mengirim ulang kapan saja untuk membuat tautan baru.",
    needsInvite: "Perlu Undangan",
    allInvitedOrOnboarded: "Semua karyawan sudah diundang atau telah menyelesaikan orientasi.",
    sending: "Mengirim...",
    sendInvite: "Kirim Undangan",
    noEmail: "Tidak ada email — tambahkan di halaman Karyawan",
    inviteSentPending: "Undangan Terkirim (Menunggu)",
    noPending: "Tidak ada undangan yang menunggu.",
    expires: "Kedaluwarsa",
    resend: "Kirim Ulang",
    onboarded: "Telah Orientasi",
    noOnboarded: "Belum ada karyawan yang menyelesaikan orientasi.",
    canLogIn: "Dapat masuk",
    statusActive: "aktif",
    statusPending: "menunggu",
    statusNeedsInvite: "perlu undangan",
    inviteCopied: "Tautan undangan disalin ke clipboard!",
  },

  // ── Employee Association ──
  association: {
    title: "Employee Association",
    description: "Manage scheduling affinities between employees.",
    affinityExplanation: "Nilai afinitas:",
    mustTogether: "= harus bekerja bersama,",
    preferTogether: "= lebih baik bersama,",
    cannotTogether: "= tidak boleh bekerja bersama,",
    preferApart: "= sebaiknya tidak bekerja bersama (gunakan sebagai pengganti -1 ketika sesekali harus menjadwalkan mereka bersama).",
    addAssociation: "+ Tambah Asosiasi",
    targetEmployee: "Karyawan Target",
    affinityLevel: "Tingkat Afinitas",
    entryDate: "Tanggal Mulai",
    expirationDate: "Tanggal Kedaluwarsa",
    typeToSearch: "Ketik untuk mencari...",
    noAssociations: "Belum ada asosiasi. Klik",
    toCreateOne: "untuk membuat satu.",
    levelMustTogether: "1.0 — Harus dijadwalkan bersama",
    levelPreferTogether: "0.5 — Lebih baik bersama",
    levelNeutral: "0.0 — Netral",
    levelPreferApart: "-0.5 — Lebih baik terpisah",
    levelMustNotTogether: "-1.0 — Tidak boleh dijadwalkan bersama",
    labelMustTogether: "Harus bersama",
    labelPreferTogether: "Lebih baik bersama",
    labelNeutral: "Netral",
    labelPreferApart: "Lebih baik terpisah",
    labelMustNotTogether: "Tidak boleh bersama",
    deleteConfirm: "Hapus asosiasi ini?",
    weekLabel: "Minggu:",
    filterPlaceholder: "Filter berdasarkan nama...",
    importFrom7shifts: "Impor dari 7shifts",
    importFromDeputy: "Import from Deputy",
    addAvailability: "+ Tambah Ketersediaan",
    import7shiftsTitle: "Impor Ketersediaan dari 7shifts",
    import7shiftsDesc: "Mengambil ketersediaan karyawan dari API 7shifts untuk rentang tanggal yang ditentukan. Ketersediaan yang ada dalam rentang tersebut akan diganti. Anda dapat mengimpor dari minggu ini hingga 8 minggu ke depan.",
    accessToken7shifts: "Token Akses 7shifts",
    importDeputyTitle: "Import Availabilities from Deputy",
    importDeputyDesc: "Fetches employee unavailability from the Deputy API and inverts it to create availability windows. Existing availability in the date range will be replaced.",
    deputyUrl: "Deputy URL",
    deputyAccessToken: "Deputy Access Token",
    deputySkippedEmployees: "employees (fetch failed)",
    accessTokenPlaceholder: "Masukkan token akses 7shifts Anda",
    from: "Dari",
    to: "Sampai",
    importing: "Mengimpor...",
    importComplete: "Impor Selesai",
    created: "Dibuat:",
    availWindows: "jendela ketersediaan",
    cleared: "Dihapus:",
    existingWindows: "jendela yang ada dalam rentang",
    skipped: "Dilewati:",
    unmatchedOrDeclined: "(tidak cocok atau ditolak)",
    outsideDateRange: "Di luar rentang tanggal:",
    addAvailWindow: "Tambah Jendela Ketersediaan",
    searchEmployee: "Cari karyawan...",
    loadingAvailability: "Memuat ketersediaan...",
    noAvailThisWeek: "Tidak ada karyawan yang memiliki ketersediaan untuk minggu ini.",
    window: "jendela",
    windows: "jendela",
    allDay: "Sepanjang Hari",
    noEmployees: "Tidak ada karyawan ditemukan.",
    addEmployeesFirst: "Tambahkan karyawan terlebih dahulu sebelum membuat asosiasi.",
  },
  // ── Employee Availability (manager) ──
  employeeAvailability: {
    title: "Employee Availability",
    noEmployees: "No employees yet.",
    addEmployeesFirst: "Add employees first before recording their availability.",
  },

  // ── Day Blackouts ──
  dayBlackouts: {
    title: "Day Rules",
    description: "Block out recurring time ranges per day of week when an employee must not be scheduled. Example: \"Alice cannot work Mondays 20:00–22:00\". These rules apply every week and the scheduler will never place a shift that overlaps a blackout.",
    addBlackout: "Add Rule",
    employee: "Employee",
    dayOfWeek: "Day of week",
    timeRange: "Time range",
    noBlackouts: "No day rules defined yet.",
    pickEmployee: "Pick an employee first.",
    invalidRange: "End time must be after start time.",
  },


  // ── Hour Restrictions ──
  hourRestrictions: {
    title: "Hour Restrictions",
    description: "Set a maximum number of hours each employee can be scheduled for in a single week. The scheduler will never assign an employee more than their cap across all locations in the same run. Leave blank for no cap.",
    searchPlaceholder: "Search employees…",
    employee: "Employee",
    maxHoursLabel: "Max hours / week",
    hoursPerWeek: "hrs/week",
    noCapPlaceholder: "No cap",
    clearCap: "Clear",
    noEmployees: "No employees found.",
    invalidValue: "Enter a number between 0 and 168, or leave blank for no cap.",
  },


  // ── Day Preferences ──
  dayPreferences: {
    title: "Preferensi Hari",
    description:
      "Beri bobot 0 hingga 1 untuk hari dalam seminggu yang lebih disukai setiap karyawan untuk bekerja. Semakin tinggi bobotnya, semakin kuat penjadwal mengutamakan karyawan tersebut untuk shift pada hari itu; nilai di bawah 1,0 adalah preferensi lunak yang masih dapat diabaikan jika tidak ada karyawan lain yang tersedia.",
    addPreference: "Tambah Preferensi",
    employee: "Karyawan",
    dayOfWeek: "Hari dalam seminggu",
    weight: "Bobot",
    hardWarning:
      "Pada bobot 1,0 ini menjadi aturan mutlak: karyawan tidak akan dijadwalkan pada hari lain, dan shift yang tidak memiliki karyawan yang memenuhi syarat akan ditandai VACANT (kosong).",
    searchPlaceholder: "Cari karyawan…",
    noPreferences: "Belum ada preferensi hari yang ditentukan.",
    pickEmployee: "Pilih karyawan terlebih dahulu.",
  },

  // ── Hour Range Preferences ──
  hourRangePreferences: {
    title: "Preferensi Rentang Jam",
    description:
      "Beri bobot 0 hingga 1 untuk rentang jam yang lebih disukai setiap karyawan untuk bekerja. Semakin tinggi bobotnya, semakin kuat penjadwal mengutamakan karyawan tersebut untuk shift pada rentang itu; nilai di bawah 1,0 adalah preferensi lunak yang masih dapat diabaikan jika tidak ada karyawan lain yang tersedia.",
    matchRule: "Sebuah shift dihitung masuk rentang ini jika setidaknya 50% durasinya berada di dalam rentang tersebut.",
    addPreference: "Tambah Preferensi",
    employee: "Karyawan",
    timeRange: "Rentang jam",
    weight: "Bobot",
    hardWarning:
      "Pada bobot 1,0 ini menjadi aturan mutlak: karyawan tidak akan dijadwalkan di luar rentang ini, dan shift yang tidak memiliki karyawan yang memenuhi syarat akan ditandai VACANT (kosong).",
    searchPlaceholder: "Cari karyawan…",
    noPreferences: "Belum ada preferensi rentang jam yang ditentukan.",
    pickEmployee: "Pilih karyawan terlebih dahulu.",
    invalidRange: "Waktu mulai dan waktu selesai tidak boleh sama.",
  },

  // ── Frequency Caps ──
  frequencyCaps: {
    title: "Batas Frekuensi",
    description:
      "Batasi berapa kali per minggu seorang karyawan dapat dijadwalkan dalam rentang jam tertentu, dengan bobot 0 hingga 1. Nilai di bawah 1,0 adalah batas lunak yang masih dapat diabaikan jika tidak ada karyawan lain yang tersedia.",
    matchRule: "Sebuah shift dihitung masuk rentang ini jika setidaknya 50% durasinya berada di dalam rentang tersebut.",
    addCap: "Tambah Batas",
    employee: "Karyawan",
    timeRange: "Rentang jam",
    maxPerWeek: "Maks / minggu",
    weight: "Bobot",
    hardWarning:
      "Pada bobot 1,0 ini menjadi batas mutlak: setelah tercapai, karyawan tidak akan dijadwalkan lagi pada rentang ini di minggu tersebut, dan shift yang tidak memiliki karyawan yang memenuhi syarat akan ditandai VACANT (kosong).",
    searchPlaceholder: "Cari karyawan…",
    noCaps: "Belum ada batas frekuensi yang ditentukan.",
    pickEmployee: "Pilih karyawan terlebih dahulu.",
    invalidRange: "Waktu mulai dan waktu selesai tidak boleh sama.",
    invalidMax: "Masukkan bilangan bulat 0 atau lebih.",
  },

  // ── Shift Templates ──
  shiftTemplates: {
    title: "Template Shift",
    addTemplate: "+ Tambah Template",
    newTemplate: "Template Shift Baru",
    chooseLayout: "Pilih tata letak jadwal:",
    weekdayWeekend: "Hari Kerja & Akhir Pekan",
    weekdayWeekendDesc: "Tentukan shift untuk hari kerja dan akhir pekan secara terpisah. Dapat diperluas ke setiap hari nanti.",
    everyDay: "Setiap Hari",
    everyDayDesc: "Tentukan shift secara individual untuk setiap hari dalam seminggu (Sen-Min).",
    weeklySchedule: "Jadwal Mingguan",
    weekdayWeekendMode: "Hari Kerja / Akhir Pekan",
    everyDayMode: "Setiap Hari",
    expandToEveryDay: "Perluas ke Setiap Hari",
    addShift: "+ Tambah Shift",
    noShiftsYet: "Belum ada shift ditambahkan.",
    headcount: "Jumlah Orang",
    nextDay: "(hari berikutnya)",
    updateShift: "Perbarui Shift",
    addShiftBtn: "Tambah Shift",
    selectRole: "-- pilih peran --",
    namePlaceholder: "mis. Standar Hari Kerja",
    noTemplatesYet: "Belum ada template shift. Buat satu di atas.",
    locationLabel: "Lokasi:",
  },

  // ── Schedule ──
  schedule: {
    prefAsteriskLabel: "Scheduled against a preference",
    prefDay: "prefers {days}",
    prefHourRange: "prefers {start}–{end}",
    prefCap: "already at {n}× this week for {start}–{end}",
    prefUnavoidable: "no one else was free",
    weekdaysShort: "Mon,Tue,Wed,Thu,Fri,Sat,Sun",
    rosterThinBanner: "{unavoidable} of {total} shifts scheduled against a preference had no one else free. You may need more staff to honor preferences and stay covered.",
    rosterThinAtGeneration: "(at generation)",
    title: "Pembuatan Jadwal",
    startLabel: "Mulai:",
    endLabel: "Selesai:",
    aiGenerate: "Buat dengan AI",
    localGenerate: "Buat Normal (dengan Strategi)",
    generating: "Membuat...",
    generationCapReachedNotice: "Anda telah menggunakan {used} dari {max} pembuatan jadwal gratis bulan ini. Upgrade untuk membuat lebih banyak.",
    reset: "Reset",
    selectTemplates: "Pilih Template Shift",
    selectTemplatesDesc: "Pilih template shift mana yang akan dijadwalkan. Template dikelompokkan berdasarkan lokasi.",
    loadingTemplates: "Memuat template...",
    noTemplatesFound: "Tidak ada template shift ditemukan. Buat template terlebih dahulu di halaman Template Shift.",
    strategy: "Strategi",
    rotation: "Rotasi",
    rotationDesc: "Mendistribusikan shift secara merata ke seluruh karyawan",
    random: "Acak",
    randomDesc: "Memilih secara acak dari karyawan yang memenuhi syarat",
    generateLocally: "Buat Normal",
    generateWithAI: "Buat dengan AI",
    template: "Template",
    templates: "Template",
    editShift: "Edit Shift",
    searchEmployee: "Cari karyawan...",
    removeShift: "Hapus Shift",
    waitingResults: "Menunggu hasil jadwal...",
    allComplete: "Jadwal Selesai - Semua lokasi telah ditinjau.",
    print: "Cetak",
    clickToEdit: "Klik shift untuk mengedit",
    approve: "Setujui",
    reject: "Tolak",
    noShiftsGenerated: "Tidak ada shift yang dibuat untuk lokasi ini.",
    lockedToastTitle: "Schedule activity in progress",
    lockedToastBody: "{locked_by} is generating or approving a schedule. Try again in {countdown}.",
    forLabel: "untuk",
    rotationHistory: "Rotasi (Riwayat 3 Bulan)",
    rotationHistoryDesc: "Mempertimbangkan jam kerja per peran selama 3 bulan terakhir. Bobot keadilan dapat disesuaikan.",
    fairnessWeight: "Bobot Keadilan",
    moreRandom: "Lebih Acak (0)",
    moreFair: "Paling Adil (1)",
    maxHours: "Jam Maksimal per Karyawan",
    maxHoursDesc: "Membatasi berapa jam seorang karyawan dapat dijadwalkan. Batas jam dan keketatan dapat disesuaikan.",
    maxHoursLimit: "Batas Jam",
    hourStrictness: "Keketatan",
    noEnforcement: "Tanpa Pembatasan (0)",
    strictEnforcement: "Strict Cap (1)",
    aiCredits: "AI Credits",
    buyCredits: "Buy Credits",
    buyCreditsPaidOnly: "Kredit hanya untuk paket berbayar — tingkatkan untuk menghapus batas bulanan.",
    balanceLabel: "saldo",
    includedThisMonth: "termasuk bulan ini",
    purchaseModalTitle: "Kredit AI",
    purchaseModalBody: "Kredit membayar Buat dengan AI dan jadwal di luar 50 yang termasuk setiap bulan. Paket dikenakan biaya ke kartu langganan Anda.",
    packButton: "Beli {amount}",
    autoReloadOptIn: "Isi ulang otomatis jumlah ini saat saldo saya habis",
    purchaseDeclined: "Kartu Anda ditolak. Perbarui di Manage billing lalu coba lagi.",
    purchaseNoCard: "Tidak ada kartu tersimpan. Tambahkan satu di Manage billing, lalu coba lagi.",
    purchaseOnHold: "Penagihan ditahan setelah pembayaran gagal. Coba lagi pembayaran terlebih dahulu.",
    purchaseSuccess: "Kredit ditambahkan.",
    creditsExhaustedMsg: "Buat dengan AI menggunakan kredit prabayar. Saldo Anda kosong — beli paket untuk melanjutkan.",
    redirectingToPayment: "Redirecting...",
    schedulesUsed: "Schedules this month",
    scheduleFreeTierUsed: "Free tier exhausted",
    scheduleQuotaExhaustedMsg: "Anda telah menggunakan 50 jadwal yang termasuk bulan ini. Jadwal tambahan dibayar dari saldo kredit yang sama — beli paket untuk melanjutkan.",
    autoReloadTitle: "Isi ulang otomatis",
    autoReloadDescription: "Opsional. Saat saldo kredit Anda turun di bawah ambang batas, kami akan mengenakan biaya isi ulang ke kartu tersimpan Anda. Nonaktif secara default; setiap biaya adalah biaya yang Anda minta.",
    autoReloadEnabled: "Diaktifkan",
    balance: "Saldo",
    threshold: "Ambang batas",
    refillAmount: "Jumlah isi ulang",
    billingOnHoldTitle: "Penagihan ditangguhkan",
    billingOnHoldBody: "Pembayaran otomatis terakhir Anda gagal pada {date}. Perbarui kartu atau coba pembayaran lagi untuk melanjutkan.",
    retryPayment: "Coba pembayaran lagi",
    retryFailed: "Percobaan gagal — perbarui kartu Anda dan coba lagi.",
    upgradeConfirmFailed: "Tidak dapat mengonfirmasi peningkatan Anda. Jika Anda telah dikenakan biaya, hubungi dukungan dan kami akan menyelesaikannya.",
    updateCard: "Perbarui kartu",
    manageBilling: "Manage Billing",
    cancellationCardTitle: "Subscription canceled",
    cancellationEndedOn: "Your subscription ended on {date}.",
    cancellationDeletionOn: "Your data will be permanently deleted on {date}.",
    reactivateSubscription: "Reactivate Subscription",
    pendingChargesTitle: "Biaya bulanan tertunda",
    pendingChargesEmpty: "Tidak ada biaya tambahan yang diproyeksikan di siklus ini.",
    pendingChargeStorage: "Kelebihan penyimpanan",
    pendingChargeEmployees: "Kelebihan karyawan",
  },

  // ── Export Schedules ──
  exportSchedules: {
    title: "Ekspor Jadwal yang Disetujui",
    description: "Lihat jadwal yang disetujui untuk tanggal yang dipilih. Ekspor ke 7Shifts atau unduh sebagai JSONL. Hari dengan titik oranye di kalender memiliki jadwal yang disetujui — klik hari mana pun untuk melihat jadwalnya. Kami berusaha mengekspor ID pengguna dengan ID peran yang terkait di sistem eksternal.",
    weekStarting: "Minggu dimulai:",
    today: "Hari Ini",
    selectAllUnexported: "Pilih Semua yang Belum Diekspor",
    exportTo7Shifts: "Ekspor ke 7Shifts",
    exporting: "Mengekspor...",
    noApproved: "Tidak ada jadwal yang disetujui untuk minggu ini.",
    noApprovedSub: "Buat dan setujui jadwal di halaman Jadwal terlebih dahulu.",
    weekOf: "Minggu",
    exported: "diekspor",
    downloadJsonl: "Unduh JSONL",
    time: "Waktu",
    notExported: "Belum diekspor",
    exportedLabel: "Diekspor",
    apiTokenTitle: "Token API 7Shifts",
    apiTokenDesc: "Masukkan token akses API 7Shifts Anda untuk mengekspor",
    tokenPlaceholder: "Token akses 7Shifts",
    exportedCount: "Diekspor",
    shiftsTo7Shifts: "shift ke 7Shifts.",
    failed: "gagal.",
  },

  // ── Employee Availability ──
  availability: {
    title: "Ketersediaan Saya",
    noProfile: "Tidak ditemukan profil karyawan untuk akun Anda. Silakan hubungi manajer Anda.",
    assignedRoles: "Peran yang Ditetapkan",
    yourLocations: "Lokasi Anda",
    addWindow: "Tambah Jendela Ketersediaan",
    addWindowDesc: "Pilih hari dan jam Anda tersedia untuk bekerja.",
    noWindows: "Belum ada jendela ketersediaan. Tambahkan satu di atas.",
  },

  // ── Import Modal ──
  importModal: {
    csv: "CSV",
    json: "JSON",
    expectedFormat: "Format yang diharapkan:",
    chooseFile: "Pilih",
    file: "File",
    uploading: "Mengunggah...",
    createdCount: "Dibuat:",
    skippedCount: "Dilewati:",
  },

  // ── Import 7Shifts Modal ──
  import7Shifts: {
    title: "Impor dari 7shifts",
    description: "Masukkan token akses 7shifts Anda untuk mengimpor perusahaan, lokasi, departemen, peran, karyawan, dan penugasan pengguna. Anda dapat membuat token akses di akun 7shifts Anda di bagian",
    devTools: "Pengaturan Perusahaan > Alat Pengembang",
    accessToken: "Token Akses",
    placeholder: "Tempel token akses 7shifts Anda",
    importingData: "Mengimpor data dari 7shifts... Ini mungkin memakan waktu.",
    importSuccess: "Impor berhasil diselesaikan",
    entity: "Entitas",
    created: "Dibuat",
    updated: "Diperbarui",
    deleted: "Dihapus",
    departments: "Departemen",
    userAssignments: "Penugasan Pengguna",
  },
  // ── Import Deputy Modal ──
  importDeputy: {
    title: "Import from Deputy",
    description: "Enter your Deputy subdomain URL and API access token to import locations, areas (roles), employees, and unavailability.",
    baseUrlLabel: "Deputy URL",
    baseUrlPlaceholder: "https://yourcompany.na.deputy.com",
    accessToken: "Access Token",
    tokenPlaceholder: "Paste your Deputy access token",
    tokenHint: "Generate a token in Deputy under Developer Tools or via the",
    importing: "Importing data from Deputy... This may take a moment.",
    success: "Deputy import completed successfully",
    entity: "Entity",
    created: "Created",
    updatedOrSkipped: "Updated / Skipped",
    deleted: "Deleted",
    unavailability: "Unavailability Rules",
    errorNoToken: "Please enter your Deputy access token.",
    errorNoUrl: "Please enter your Deputy URL (e.g. https://yourcompany.na.deputy.com).",
  },


  // ── DataTable ──
  dataTable: {
    addRow: "+ Tambah Baris",
  },

  // ── Search ──
  search: {
    noMatches: "Tidak ada hasil",
    searchEmployees: "Cari karyawan...",
  },
  // ── GDPR / Privacy ──
  gdpr: {
    dataPrivacy: "Data & Privacy",
    yourConsents: "Your Consents",
    consentType: "Consent Type",
    version: "Version",
    grantedAt: "Granted",
    revokedAt: "Revoked",
    exportData: "Export Your Data",
    exportDataDesc: "Download a copy of all your personal data in JSON format.",
    exportBtn: "Download My Data",
    exporting: "Exporting...",
    deleteAccount: "Delete Your Account",
    deleteAccountDesc: "Permanently delete your account and all associated data. This action cannot be undone.",
    deleteConfirmPrompt: "Type DELETE to confirm:",
    deleteBtn: "Delete My Account",
    deleting: "Deleting...",
    privacyPolicy: "Privacy Policy",
    termsOfService: "Terms of Service",
    dpa: "Data Processing Agreement",
    acceptPrivacy: "I accept the",
    acceptTerms: "I accept the",
    processors: "Third-Party Data Processors",
    processorName: "Processor",
    processorPurpose: "Purpose",
    processorLocation: "Location",
    effectiveDate: "Effective Date",
    backToLogin: "Back to Login",
  },
  // ── Landing ──
  landing: {
    badge: "Penjadwalan Karyawan Algoritmik & Berbasis AI",
    heroTitle: "Penjadwalan Cerdas,",
    heroTitleAccent: "Tanpa Repot",
    heroDesc: "Buat jadwal shift mingguan yang optimal dengan strategi algoritmik atau AI. Otomatis menghormati ketersediaan, peran, tingkat keterampilan, dan preferensi tim.",
    getStarted: "Mulai",
    viewPricing: "View Pricing",
    viewDemo: "View Demo",

    exploreDashboard: "Explore every manager screen →",

    featuresLink: "Manager tour",
    demoTitle: "See It in Action",
    demoDesc: "Watch a quick walkthrough of how Wiz Scheduler helps you build optimized schedules in minutes.",
    demoFallback: "Your browser does not support the video tag.",
    featuresTitle: "Semua yang Anda Butuhkan",
    featuresDesc: "Dari manajemen multi-lokasi hingga optimasi berbasis AI, Wiz Scheduler menangani kerumitan agar Anda tidak perlu.",
    strategiesTitle: "Strategi Penjadwalan",
    strategiesDesc: "Pilih pendekatan yang sesuai dengan bisnis Anda. Campurkan strategi di berbagai lokasi.",
    pricingTitle: "Harga Sederhana dan Transparan",
    pricingDesc: "Mulai gratis. Bayar hanya saat berkembang. Tanpa biaya tersembunyi.",
    allInOnePlan: "Paket All-In-One",
    pricePerMonth: "/ bulan",
    basePlanDesc: "Langganan dasar per grup kepemilikan. Termasuk 50 jadwal per bulan, 0.5 GB penyimpanan, dan 1K karyawan.",
    aiCredits: "Paket kredit AI",
    aiPackFrom: "Mulai dari $10",
    compSchedules: "Jadwal gratis / bulan",
    storageIncluded: "Penyimpanan termasuk",
    employeesIncluded: "Karyawan termasuk",
    normalStrategiesNote: "Rotation, Max Hours, dan Random selalu termasuk. AI Generate berjalan dengan paket kredit prabayar.",
    aiScheduling: "Penjadwalan AI",
    included: "Termasuk",
    overage: "Kelebihan",
    aiOverageNote: "Biaya token Claude Sonnet 5 ($2/M masuk, $10/M keluar) dengan markup 130%, didebit dari paket prabayar $10, $25, atau $50.",
    schedules: "Jadwal",
    schedulesOverageNote: "Semua jenis generasi dihitung. Ditagih dalam blok 50 setelah 50 gratis.",
    storage: "Penyimpanan",
    storageOverageNote: "Semua data: karyawan, jadwal, ketersediaan, shift, dan riwayat.",
    employees: "Karyawan",
    employeesOverageNote: "Per grup kepemilikan. Ditagih dalam blok 1K setelah kuota gratis.",
    exampleTitle: "Contoh: Jaringan Kopi dengan 50 Lokasi",
    exampleItem: "Item",
    exampleUsage: "Penggunaan",
    exampleCost: "Biaya",
    exampleBase: "Langganan dasar",
    exampleSchedules: "Jadwal yang dibuat",
    exampleSchedulesUsage: "200 / month",
    exampleSchedulesCost: "$0.30 (150 overage = 3 blocks)",
    exampleAI: "AI Generate (sesekali)",
    exampleAIUsage: "~$1.50 dalam token",
    exampleAICost: "$1.95 dari kredit",
    exampleStorage: "Penyimpanan",
    exampleStorageUsage: "~0.3 GB",
    exampleStorageCost: "$0.00 (dalam 0.5 GB gratis)",
    exampleEmployees: "Karyawan",
    exampleEmployeesUsage: "2.500",
    exampleEmployeesCost: "$2.00 (1.5K overage = 2 blocks)",
    exampleTotal: "Total Bulanan",
    complianceTitle: "Privasi & Kepatuhan",
    complianceDesc: "Dibangun dengan memperhatikan kepatuhan GDPR. Data Anda, kendali Anda.",
    ctaTitle: "Siap Menyederhanakan Penjadwalan?",
    ctaDesc: "Bergabunglah dengan ribuan manajer yang menghemat berjam-jam setiap minggu.",
    ctaBtn: "Buat Akun",
    featAITitle: "Pembuatan Jadwal AI",
    featAIDesc: "Claude AI menganalisis ketersediaan, keterampilan, dan dinamika tim untuk membuat jadwal optimal dalam hitungan detik.",
    featStrategiesTitle: "Beragam Strategi",
    featStrategiesDesc: "Rotasi, keadilan riwayat 3 bulan, batas jam maksimum, atau acak murni. Pilih yang cocok untuk setiap lokasi.",
    featMultiLocTitle: "Multi-Lokasi",
    featMultiLocDesc: "Kelola wilayah, lokasi, dan grup kepemilikan. Setiap lokasi memiliki template shift dan jadwal sendiri.",
    featSelfServiceTitle: "Layanan Mandiri Karyawan",
    featSelfServiceDesc: "Karyawan mengatur ketersediaan mereka sendiri melalui tautan undangan. Manajer melihat semuanya di satu tempat.",
    featHourCapsTitle: "Per-Employee Hour Caps",
    featHourCapsDesc: "Set a maximum weekly hours limit for each employee. The scheduler enforces the cap across all locations in a single run — no manual tracking needed.",
    featDayRulesTitle: "Day Blackout Rules",
    featDayRulesDesc: "Block out recurring time ranges per day of week when an employee must not be scheduled. For example, no Mondays 8 PM-10 PM — enforced automatically every week.",
    featDeputyTitle: "Deputy Integration",
    featDeputyDesc: "Import locations, areas, employees, and unavailability from Deputy. Unavailability is automatically inverted into availability windows for scheduling.",
    feat7shiftsTitle: "Integrasi 7shifts",
    feat7shiftsDesc: "Impor karyawan, peran, dan ketersediaan dari 7shifts. Ekspor jadwal yang disetujui dengan satu klik.",
    featGDPRTitle: "Sesuai GDPR",
    featGDPRDesc: "Ekspor data, penghapusan akun, pelacakan persetujuan, redaksi log, dan kebijakan retensi data otomatis.",
    featAffinitiesTitle: "Afinitas Tim",
    featAffinitiesDesc: "Atur preferensi siapa yang harus (atau tidak harus) bekerja bersama. Batasan keras dan lunak didukung.",
    featRoleEquivTitle: "Kesetaraan Peran",
    featRoleEquivDesc: "Kelompokkan peran serupa agar karyawan dapat mengisi shift di posisi setara secara otomatis.",
    featLangsTitle: "19 Bahasa",
    featLangsDesc: "Lokalisasi UI penuh dalam bahasa Inggris, Mandarin, Hindi, Spanyol, Prancis, Arab, Jepang, dan 12 bahasa lainnya.",
    stratRotation: "Rotasi",
    stratRotationTag: "GRATIS",
    stratRotationDesc: "Mendistribusikan shift secara merata di antara karyawan dalam jadwal saat ini. Memberikan penalti untuk penugasan berulang ke peran yang sama. Menghormati afinitas karyawan: batasan keras ditegakkan, preferensi lunak memengaruhi penilaian.",
    stratRotationHistory: "Rotasi (Riwayat 3 Bulan)",
    stratRotationHistoryTag: "GRATIS",
    stratRotationHistoryDesc: "Mempertimbangkan total jam kerja per peran selama 3 bulan terakhir. Bobot keadilan yang dapat disesuaikan dari acak (0) hingga sangat adil (1). Menghormati afinitas karyawan: batasan keras ditegakkan, preferensi lunak memengaruhi penilaian.",
    stratMaxHours: "Jam Maksimum Per Karyawan",
    stratMaxHoursTag: "GRATIS",
    stratMaxHoursDesc: "Membatasi karyawan mana pun hingga X jam per jadwal. Batas jam yang dapat disesuaikan (4-60 jam) dan tingkat keketatan (dari preferensi lunak hingga batas keras). Menghormati afinitas karyawan: batasan keras ditegakkan, preferensi lunak memengaruhi penilaian.",
    stratAI: "AI Generate",
    stratAITag: "KREDIT PRABAYAR",
    stratAIDesc: "Claude AI membaca konteks lengkap (ketersediaan, keterampilan, afinitas, persyaratan shift) dan menghasilkan jadwal yang dioptimalkan. Menghormati semua batasan afinitas. Terbaik untuk skenario kompleks.",
    // Inputs (what the scheduler considers)
    inputsTitle: "Apa yang Dipertimbangkan Penjadwal",
    inputsDesc: "Setiap draf, dari setiap strategi, diperiksa terhadap aturan yang sama. Di sini namanya sama dengan yang digunakan di halaman Aturan Penjadwalan pada aplikasi.",
    inputsHardHeading: "Selalu diberlakukan",
    inputsSoftHeading: "Diberi bobot 0 hingga 1. Pada 1,0 sebuah preferensi menjadi aturan.",
    inputAvailabilityTitle: "Ketersediaan karyawan",
    inputAvailabilityDesc: "Shift hanya ditempatkan dalam jendela waktu yang diberikan setiap karyawan. Tidak ada ketersediaan yang tercatat berarti tidak pernah dijadwalkan. Jam yang sudah dipesan di lokasi lain dalam proses yang sama akan ditahan.",
    inputRolesTitle: "Peran dan tingkat keterampilan",
    inputRolesDesc: "Seorang karyawan harus memegang peran shift tersebut, atau peran yang setara. Tingkat keterampilan yang lebih tinggi diutamakan ketika beberapa orang memenuhi syarat.",
    inputHourRestrictionsTitle: "Hour Restrictions",
    inputHourRestrictionsDesc: "Batas jam mingguan per karyawan, dijaga di seluruh lokasi dalam proses yang sama.",
    inputDayRulesTitle: "Day Rules",
    inputDayRulesDesc: "Rentang blokir berulang per hari dalam seminggu. Penjadwal tidak pernah menempatkan shift yang melintasinya.",
    inputMinRestTitle: "Istirahat minimum",
    inputMinRestDesc: "Jeda per lokasi antara shift pada hari yang berbeda, sehingga tidak ada yang tutup lalu langsung buka.",
    inputAssociationTitle: "Employee Association",
    inputAssociationDesc: "Pasangan yang tidak boleh bekerja bersama tidak pernah ditempatkan pada shift yang sama. Pasangan yang harus bekerja bersama, yang sebaiknya bersama, dan yang lebih suka terpisah mengarahkan siapa yang dipilih.",
    inputDayPrefsTitle: "Preferensi Hari",
    inputDayPrefsDesc: "Hari apa dalam seminggu yang disukai setiap karyawan.",
    inputHourRangeTitle: "Preferensi Rentang Jam",
    inputHourRangeDesc: "Waktu apa dalam sehari yang disukai setiap karyawan. Sebuah shift dihitung ketika setidaknya setengahnya jatuh dalam rentang tersebut.",
    inputFreqCapsTitle: "Batas Frekuensi",
    inputFreqCapsDesc: "Berapa kali seminggu seorang karyawan bekerja dalam rentang jam tertentu.",
    inputsTemplatesNote: "Template shift dan pengecualian tanggal tertentu menentukan apa yang perlu diisi. Setiap draf divalidasi ulang setelah pembuatan, dan shift mana pun yang bertentangan dengan preferensi lunak ditandai untuk ditinjau.",
    inputsNotYet: "Belum dipertimbangkan: tarif gaji, premi lembur, senioritas. Istirahat minimum adalah satu-satunya aturan hukum ketenagakerjaan yang dibangun ke dalamnya.",
    compDataExportTitle: "Ekspor Data",
    compDataExportDesc: "Unduh semua data Anda sebagai JSON kapan saja.",
    compErasureTitle: "Hak untuk Dihapus",
    compErasureDesc: "Hapus akun dan semua data Anda secara permanen.",
    compConsentTitle: "Pelacakan Persetujuan",
    compConsentDesc: "Setiap persetujuan dicatat dengan timestamp dan IP.",
    compNoNamesTitle: "Tanpa Nama ke AI",
    compNoNamesDesc: "Nama karyawan tidak pernah dikirim ke LLM.",
  },

  features: {
    pageTitle: "Setiap layar yang digunakan manajer, dari ujung ke ujung",
    pageIntro:
      "Ikuti tur terpandu pengalaman manajer Wiz Scheduler: dari mengatur perusahaan dan cabang Anda, mendefinisikan peran dan template shift, hingga menghasilkan jadwal mingguan yang dioptimalkan AI dan mengekspornya.",
    backToHome: "← Kembali ke beranda",
    tocTitle: "Telusuri layar",
    ctaTitle: "Siap mencoba Wiz Scheduler?",
    ctaDesc: "Mulai dari paket gratis — tanpa kartu kredit.",
    ctaBtn: "Buat akun",
    pages: {
      dashboard: {
        title: "Dasbor",
        desc: "Titik awal Anda. Lihat penggunaan sekilas: status periode penagihan, jadwal yang dibuat, kredit AI terpakai, dan penyimpanan. Pintasan cepat ke layar yang paling sering Anda kunjungi.",
      },
      company: {
        title: "Perusahaan",
        desc: "Atur grup kepemilikan: nama perusahaan, logo, branding, dan pengaturan tingkat tinggi yang berlaku di setiap wilayah dan cabang.",
      },
      regions: {
        title: "Wilayah",
        desc: "Kelompokkan cabang berdasarkan wilayah untuk pelaporan dan manajemen. Berguna untuk operator multi-negara bagian, multi-negara, atau franchise yang menginginkan lapisan di atas masing-masing cabang.",
      },
      locations: {
        title: "Cabang",
        desc: "Kelola setiap lokasi fisik yang Anda jadwalkan. Setiap cabang memiliki zona waktu, alamat, wilayah, dan daftar karyawan tersendiri — dan dijadwalkan secara independen oleh AI.",
      },
      roles: {
        title: "Peran",
        desc: "Tentukan jabatan yang dapat ditugaskan kepada karyawan. Peran sepenuhnya dapat disesuaikan per perusahaan — tidak ada nama yang dikodekan secara permanen, jadi Wiz Scheduler menyesuaikan diri dengan kosakata industri apa pun.",
      },
      "role-equivalents": {
        title: "Padanan peran",
        desc: `Petakan peran yang dapat saling menggantikan. Memungkinkan penjadwal memperlakukan "Barista" dan "Baker" sebagai dapat dipertukarkan saat mengisi shift, sambil tetap menghormati perbedaan tingkat keterampilan.`,
      },
      employees: {
        title: "Karyawan",
        desc: "Daftar utama. Atur peran yang ditugaskan, tingkat keterampilan, cabang tempat bekerja, batas jam, dan ketersediaan setiap orang. Pengeditan inline membuat pembaruan massal cepat.",
      },
      "hour-restrictions": {
        title: "Pembatasan jam",
        desc: "Berlakukan batas atas dan bawah jam mingguan per karyawan. Berguna untuk visa pelajar, kesepakatan paruh waktu, dan anggaran lembur. AI tidak akan pernah menjadwalkan di luar batas ini.",
      },
      "day-blackouts": {
        title: "Hari tidak tersedia",
        desc: "Blokir hari penuh ketika cabang tutup atau karyawan tidak tersedia. Hari libur, cuti, hari pelatihan — penjadwal menghormatinya secara otomatis.",
      },
      "employee-onboarding": {
        title: "Onboarding karyawan",
        desc: "Undang karyawan baru melalui email. Mereka mengisi sendiri ketersediaan dan data pribadi — Anda tetap fokus menjalankan bisnis, bukan mengejar data.",
      },
      "employee-association": {
        title: "Asosiasi karyawan",
        desc: "Tandai karyawan yang bekerja baik bersama (atau tidak boleh dipasangkan). AI menggunakan preferensi ini sebagai panduan lunak saat menugaskan shift.",
      },
      "shift-templates": {
        title: "Template shift",
        desc: "Tentukan pola shift mingguan berulang untuk setiap cabang: peran apa yang dibutuhkan, berapa banyak masing-masing, pada hari dan jam berapa. Cetak biru yang akan diisi AI.",
      },
      schedule: {
        title: "Jadwal",
        desc: "Buat jadwal mingguan yang dioptimalkan dengan satu klik. Pilih strategi algoritma (Rotasi, Jam Maks, Acak) atau AI. Tinjau hasil per cabang, edit inline, dan publikasikan.",
      },
      "export-schedules": {
        title: "Ekspor jadwal",
        desc: "Unduh jadwal yang dipublikasikan sebagai CSV atau PDF, atau kirim langsung ke 7shifts dan Deputy. Alur penggajian dan POS Anda yang ada tetap tidak berubah.",
      },
      "data-privacy": {
        title: "Privasi data",
        desc: "Alat ramah GDPR untuk ekspor, penghapusan, dan persetujuan data. Tampilan manajer untuk setiap permintaan subjek data dan statusnya.",
      },
    },
  },

  cancellationBanner: {
    title: "Subscription ended.",
    body: "Your subscription ended on {endDate}. Your data will be permanently deleted on {deleteDate} unless you reactivate.",
  },
  // ── Check-In ──
  checkIn: {
    qrTitle: "Kode Check-in",
    qrDesc: "Karyawan memindai ini untuk check-in. Kode berubah setiap kali dipindai, jadi foto kode ini tidak berguna bagi siapa pun yang tidak berada di sini.",
    selectLocation: "Lokasi",
    checkedInToday: "Check-in hari ini",
    reportTitle: "Laporan Check-in",
    reportDesc: "Seberapa jauh dari waktu mulai terjadwal setiap karyawan tiba, selama {days} hari terakhir. Di bawah garis berarti lebih awal, di atas garis berarti terlambat.",
    allEmployees: "Semua karyawan",
    filterEmployee: "Karyawan",
    minutesLate: "Menit dari waktu mulai terjadwal",
    date: "Tanggal",
    noData: "Belum ada check-in yang tercatat.",
    checkingIn: "Sedang memproses check-in Anda...",
    successMatched: "Check-in berhasil. Anda {minutes} menit {direction}.",
    successOnTime: "Check-in berhasil, tepat waktu.",
    directionEarly: "lebih awal",
    directionLate: "terlambat",
    successNoShift: "Check-in berhasil, tetapi Anda tidak terjadwal di sekitar waktu ini. Manajer Anda akan melihatnya.",
    successWrongLocation: "Check-in berhasil, tetapi shift Anda hari ini ada di lokasi lain.",
    successDuplicate: "Anda sudah check-in hari ini. Ini juga akan dicatat.",
    codeExpired: "Kode itu sudah digunakan. Pindai kode baru di layar.",
    failed: "Check-in gagal. Tanyakan kepada manajer Anda.",
    noEmployeeRecord: "Akun ini tidak terhubung dengan data karyawan mana pun.",
    scanPrompt: "Pindai kode check-in yang ditampilkan di lokasi Anda untuk check-in.",
    managerLoadFailed: "Tidak dapat memuat kode check-in. Periksa apakah paket Anda mencakup check-in, atau coba lagi.",
    statusColumn: "Status",
    statusMatched: "Cocok",
    statusNoShift: "Tanpa shift",
    statusWrongLocation: "Lokasi salah",
    statusDuplicate: "Duplikat",
    attestationTitle: "Shift yang dikonfirmasi manajer",
    attestationDesc: "Porsi jam yang dibayar dan dikonfirmasi oleh manajer alih-alih check-in, per lokasi, selama {days} hari terakhir. Angka tinggi layak ditinjau, bukan dihukum.",
  },

  // ── Payroll (#78) ──
  payroll: {
    title: "Jam Penggajian",
    desc: "Shift yang disetujui dan ada check-in menjadi jam yang dibayar. Shift tanpa check-in menunggu di bawah untuk dikonfirmasi manajer.",
    rangeStart: "Dari",
    rangeEnd: "Sampai",
    location: "Lokasi",
    allLocations: "Semua lokasi",
    exceptionsTitle: "Perlu konfirmasi",
    exceptionsDesc: "Shift ini sudah dijadwalkan dan disetujui, tetapi tidak ada yang check-in. Konfirmasi yang benar-benar dikerjakan agar dibayar.",
    exceptionsEmpty: "Setiap shift yang disetujui pada rentang ini punya check-in.",
    attest: "Konfirmasi dikerjakan",
    attestReason: "Alasan (opsional)",
    attestConfirm: "Konfirmasi",
    attestCancel: "Batal",
    attestFailed: "Tidak bisa mengonfirmasi shift itu. Muat ulang lalu coba lagi.",
    entriesTitle: "Jam yang dibayar",
    entriesEmpty: "Belum ada jam yang dibayar pada rentang ini.",
    columnEmployee: "Karyawan",
    columnDate: "Tanggal",
    columnLocation: "Lokasi",
    columnRole: "Peran",
    columnStart: "Mulai",
    columnEnd: "Selesai",
    columnPaidHours: "Jam dibayar",
    columnSource: "Sumber",
    columnCheckedInAt: "Check-in",
    columnLateness: "vs. jam mulai",
    sourceCheckedIn: "Check-in",
    sourceAttested: "Dikonfirmasi manajer",
    attestedBy: "Dikonfirmasi oleh {name} pada {date}",
    latenessEarly: "{minutes} mnt lebih awal",
    latenessLate: "{minutes} mnt terlambat",
    latenessOnTime: "Tepat waktu",
    approveSelected: "Setujui yang dipilih",
    approveAll: "Setujui semua pada rentang ini",
    approved: "Disetujui",
    approvedCount: "{count} entri disetujui.",
    download: "Unduh CSV",
    downloadDesc: "Hanya jam yang disetujui. Setiap entri diekspor sekali.",
    nothingToExport: "Tidak ada yang baru untuk diekspor. Setujui dulu sebagian jam.",
    alreadyExported: "Diekspor {date}",
    totalHours: "{hours} jam dari {count} shift",
    paidPlanOnly: "Ekspor penggajian adalah fitur paket berbayar. Tingkatkan untuk mengaktifkannya.",
    loadFailed: "Tidak bisa memuat jam penggajian. Coba lagi.",
  },
} as const;

export default ind;
