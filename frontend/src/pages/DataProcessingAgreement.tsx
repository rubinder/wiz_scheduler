          ...
          {policy && (
            <>
              ...
              {policy.processors && policy.processors.length > 0 && (
                <div className="mt-6">
                  <h2 className={`${m.text.display} font-display text-lg font-semibold mb-3`}>
                    {t.gdpr.processors}
                  </h2>
                    ...
                    <table className="w-full text-sm text-start">
                      <thead>
                        <tr className={`border-b ${m.rule.line}`}>
                          <th className={`py-2 px-3 ${m.text.meta} font-medium text-start`}>
                            {t.gdpr.processorName}
                          </th>
                          <th className={`py-2 px-3 ${m.text.meta} font-medium text-start`}>
                            {t.gdpr.processorPurpose}
                          </th>
                          <th className={`py-2 px-3 ${m.text.meta} font-medium text-start`}>
                            {t.gdpr.processorLocation}
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {policy.processors.map((proc, idx) => (
                          <tr key={idx} className={`border-b ${m.rule.line}`}>
                            <td className={`py-2 px-3 ${m.text.body}`}>{proc.name}</td>
                            <td className={`py-2 px-3 ${m.text.body}`}>{proc.purpose}</td>
                            <td className={`py-2 px-3 ${m.text.body}`}>{proc.location}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
